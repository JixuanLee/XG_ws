...
PathFollow::PathFollow(ros::NodeHandle& nh): listener_(buffer_)
{
    GetParams(nh);

    // subscribe
    SceneModeSubTopic = "/veh_status"; // 临时：为了XG验收，发布写在了veh_io里
    OdomSub = nh.subscribe(OdomSubTopic, 10, &PathFollow::OdomCallback, this);//位姿+车辆自身速度
    PathSub = nh.subscribe(PathSubTopic, 10, &PathFollow::PathCallback, this);//map坐标系下的路径
    DesPointSub = nh.subscribe(DesPointSubTopic, 10, &PathFollow::DesPointCallback, this); // 来自Rviz指定的全局路径的终点 
    SceneModeSub = nh.subscribe(SceneModeSubTopic, 10, &PathFollow::SceneModeCallback, this); // 临时：为了XG验收，接收高速场景变换信号，后续可以整合为决策层接口
...
void PathFollow::GetParams(ros::NodeHandle& nh)
{
    ...
    GetDynamicParams(nh, HighSpeedParams);
...
void PathFollow::GetDynamicParams(ros::NodeHandle& nh, DynamicParamsForPF &s)
{
    std::string sNameSpace = std::string("/path_follow/") + s.name;
...
void PathFollow::UpdataDynamicParams(DynamicParamsForPF &sFrom)
{
    UsingParams = sFrom;
}

void PathFollow::OdomCallback(const nav_msgs::Odometry& msg)//位姿回调函数，获取当前车辆位姿
{
...
void PathFollow::PathCallback(const nav_msgs::Path& msg)//路径回调函数，获取路径信息
{
...
void PathFollow::DesPointCallback(const geometry_msgs::PoseStampedConstPtr &msgPtr)
{
...
void PathFollow::SceneModeCallback(const sensor_msgs::BatteryStateConstPtr &msgPtr)
{
...
bool PathFollow::get_transform(const std::string & target_frame_id, const std::string & source_frame_id,
...
std::string PathFollow::remove_first_slash(std::string frame_id)
{
