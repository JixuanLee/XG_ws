...
PathFollowOC::PathFollowOC(ros::NodeHandle& nh): listener_(buffer_)
{
    ...
    // subscribe
    SceneModeSubTopic = "/veh_status_no_use_so_change_this_name_to_whatelse"; // 临时：为了XG验收，发布写在了veh_io里
    OdomSub = nh.subscribe(OdomSubTopic, 10, &PathFollowOC::OdomCallback, this);//位姿+车辆自身速度
    PathSub = nh.subscribe(PathSubTopic, 10, &PathFollowOC::PathCallback, this);//map坐标系下的路径
    DesPointSub = nh.subscribe(DesPointSubTopic, 10, &PathFollowOC::DesPointCallback, this); // 来自Rviz指定的全局路径的终点 
    SceneModeSub = nh.subscribe(SceneModeSubTopic, 10, &PathFollowOC::SceneModeCallback, this); // 临时：为了XG验收，接收高速场景变换信号，后续可以整合为决策层接口
