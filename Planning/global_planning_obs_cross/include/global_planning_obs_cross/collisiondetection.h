...
class CollisionDetection {
  ...
  */
  template<typename T> 
  bool isTraversable(const T* node, bool check_obs = false) {
    ...
    */
    ...
    if (check_obs)// 单独为了区分是否是需要越障的障碍物而添加的
    {
      return (grid->data[node->getIdx()] >= 40 && grid->data[node->getIdx()] <= 70);//sjd
    }
    else
    {
      // 2D collision test: 2D网格就直接检测环境网格中的占据情况
      if (t == 99) {
        return (grid->data[node->getIdx()] >= 0 && grid->data[node->getIdx()] <= 70);//sjd
      }
      return configurationTest(x, y, t);//如果true，表明为自由网格，通行代价为0；有障碍则返回false，通行代价值为1
    }
