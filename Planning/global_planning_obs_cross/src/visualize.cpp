#include "global_planning_obs_cross/visualize.h"
...
void Visualize::publishNode3DPose(Node3D& node) {
  ...
  // pose.header.seq = 0;
  pose.pose.position.x = node.getX() * Constants::cellSize + ori_x;
  pose.pose.position.y = node.getY() * Constants::cellSize + ori_y;
...
void Visualize::publishNode3DPoses(Node3D& node) {
  ...
  pose.position.x = node.getX() * Constants::cellSize + ori_x;
  pose.position.y = node.getY() * Constants::cellSize + ori_y;
...
void Visualize::publishNode2DPose(Node2D& node) {
  ...
  pose.header.stamp = ros::Time::now();
  // pose.header.seq = 0;
  pose.pose.position.x = (node.getX() + 0.5) * Constants::cellSize + ori_x;
  pose.pose.position.y = (node.getY() + 0.5) * Constants::cellSize + ori_y;
...
void Visualize::publishNode2DPoses(Node2D& node) {
  if (node.isDiscovered()) {
    geometry_msgs::Pose pose;
    pose.position.x = (node.getX() + 0.5) * Constants::cellSize + ori_x;
    pose.position.y = (node.getY() + 0.5) * Constants::cellSize + ori_y;
...
void Visualize::publishNode3DCosts(Node3D* nodes, int width, int height, int depth) {
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    // set a new maximum
    if (values[i] > 0 && values[i] > max && values[i] != 100000) {
      max = values[i];
    }
   }
  ...
  for (int i = 0; i < width * height; ++i) {
    ...
    if (values[i] != 100000) {
     ...
     //填充costCube结构体，存入costCubes
      ...
      // center in cell +0.5
      costCube.pose.position.x = (i % width + 0.5) * Constants::cellSize + ori_x;
      costCube.pose.position.y = ((i / width) % height + 0.5) * Constants::cellSize + ori_y;
