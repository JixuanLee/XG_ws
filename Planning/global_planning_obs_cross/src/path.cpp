 ...
 * 注：该类主要用于与RVIZ交互
 */
#include "global_planning_obs_cross/path.h"
...
void Path::addSegment(const Node3D& node) {
  ...
  vertex.pose.position.x = node.getX() * Constants::cellSize + ori_x;
  vertex.pose.position.y = node.getY() * Constants::cellSize + ori_y;
...
void Path::addNode(const Node3D& node, int i) {
  ...
  pathNode.pose.position.x = node.getX() * Constants::cellSize + ori_x;
  pathNode.pose.position.y = node.getY() * Constants::cellSize + ori_y;
...
void Path::addVehicle(const Node3D& node, int i) {
  ...
  pathVehicle.pose.position.x = node.getX() * Constants::cellSize + ori_x;
  pathVehicle.pose.position.y = node.getY() * Constants::cellSize + ori_y;
