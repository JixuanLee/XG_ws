...
class Path {
 public:
  ...
  // sjd 加入 考虑地图原点
  void setMapOri(float x, float y){ori_x = x; ori_y = y;}
  ...
  */
  ...
  void clear();//清除路径
  /// Publishes the path
  void publishPath() { std::reverse(path.poses.begin(), path.poses.end()); pubPath.publish(path); }//将path通过话题发布出去
