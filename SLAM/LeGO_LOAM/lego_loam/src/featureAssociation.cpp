...
public:
    FeatureAssociation() : nh("~")
    {
        ...
        subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/segmented_cloud", 1, &FeatureAssociation::laserCloudHandler, this);            // 订阅分割之后的点云
        subLaserCloudInfo = nh.subscribe<cloud_msgs::cloud_info>("/segmented_cloud_info", 1, &FeatureAssociation::laserCloudInfoHandler, this); // 订阅的分割点云含有图像的深度信息
        subOutlierCloud = nh.subscribe<sensor_msgs::PointCloud2>("/outlier_cloud", 1, &FeatureAssociation::outlierCloudHandler, this);          // 非聚类的点
        subImu = nh.subscribe<sensor_msgs::Imu>(imuTopic, 50, &FeatureAssociation::imuHandler, this);                                           // IMU传感器的消息
        ...
        pubLaserOdometry = nh.advertise<nav_msgs::Odometry>("/laser_to_init", 5); // 原为laser_odom_to_init
    ...
    void initializationValue()
    {
        ...
        laserOdometry.header.frame_id = "/vodom"; // 原为camera_init
        laserOdometry.child_frame_id = "/vveh";   // 原为laser_odom

        // 粗估计的完全可以不发tf变换，尤其是在需要改成odom->veh的情况下。
        // laserOdometryTrans.frame_id_ = "/vodom";      // 原为camera_init
        // laserOdometryTrans.child_frame_id_ = "/vveh";// 原为laser_odom
    ...
    // 是imu回调函数里部分内容的封装
    // 将原始的在车身坐标系下的线加速度xyz通过rpy旋转，得到了在相对于东北天坐标系（这个坐标系是rpy为0的坐标系）的线加速度xyz
    void AccumulateIMUShiftAndRotation()
    {
        ...
        // 小于时间阈值后，更新偏移量与旋转。更新imu的偏移量、速度、角速度引起的旋转
    ...
    // 以下是四个回调函数，其中imu的回调函数用到了以上AccumulateIMUShiftAndRotation函数
    // 回调函数都是暂时将输出存到缓存中，imu除外
    void imuHandler(const sensor_msgs::Imu::ConstPtr &imuIn)
    {
        ...
        // 把FLU坐标系换成了LUF坐标系，而且认为有g时加速度应该为0
    ...
    // 1.1 去除运动畸变
    // 其实是把(k，k+1)时刻扫描到的激光雷达点投影(统一)至k时刻，就是那个间隔为0.1s的扫描的起始时刻，
    // 但是是否应该投影至起始时刻有待商榷，所有校正的点都放到了segmentedCloud中，之后计算粗糙度，标记阻塞点，提取特征三步操作等都是使用其中的数据
    void adjustDistortion()
    {
        ...
        {
            point.x = segmentedCloud->points[i].y; // 旋转xyz -> yzx
    ...
    // 1.3 标记出远点和离散点
    // 此处标记的瑕疵点，更多就是遮挡点了，而在此处被标记的点将不会被纳入到特征点的提取中。
    ...
    // 1.4 特征提取
    // ​ 特征点的提取过程和loam 一样。提取曲率最大的两个点作为边特征点，然后在剩下的里面找到 20个作为备选的20个边特征点。
    // 但是面特征点选取不同，此处的面特征点选取放在了被标记的地面点中，在里面选取了曲率最小的4个点作为面特征点，剩下的作为备选点，
    // 如果在某处点云过于密集，则会降采样处理，这样就保证了选取的特征点不在过于密集的地方。
            ...
            for (int j = 0; j < 6; j++)
            {
                ...
                for (int k = ep; k >= sp; k--)
                {
                        ...
                        segInfo.segmentedCloudGroundFlag[ind] == false)
                    {
                        ...
                        // 将已经选取的边特征点标记，这些点不会被纳入特征点的选取。并且把临近的点也标记，后续不纳入特征点选取
    ...
    void publishCloud()
    {
        ...
        if (pubCornerPointsSharp.getNumSubscribers() != 0)
        {
            ...
            laserCloudOutMsg.header.frame_id = "vrslidar_link"; // 原始为“camera”
    ...
    void checkSystemInitialization()
    {
        ...
        laserCloudCornerLast2.header.frame_id = "vrslidar_link"; // 原始为“camera”
        pubLaserCloudCornerLast.publish(laserCloudCornerLast2);

        sensor_msgs::PointCloud2 laserCloudSurfLast2;
        pcl::toROSMsg(*laserCloudSurfLast, laserCloudSurfLast2);
        laserCloudSurfLast2.header.stamp = cloudHeader.stamp;
        laserCloudSurfLast2.header.frame_id = "vrslidar_link"; // 原始为“camera”
    ...
    // 2.1 更新初始位姿
    // ​这个函数主要功能是对激光里程计初始化，在loam 代码中，激光里程计是将第一帧作为参考帧。如果系统没有初始化，则一直将当前帧作为参考帧。
    // 函数根据 IMU积分的结果，计算出一个初始位姿transformCur，这个位姿指的是雷达旋转一圈后发生的相对位姿变换。—本文中涉及到 IMU 的内容都没有看懂。
    ...
    // 2.4 发布雷达里程计消息
    // 发布激光里程计位姿和 TF 变换。这里的坐标变换实际上是 odom 相对于地图原点的，所以会有累计误差出现。
    ...
    void adjustOutlierCloud()
    {
        ...
        for (int i = 0; i < cloudSize; ++i)
        {
            point.x = outlierCloud->points[i].y; // 旋转xyz -> yzx
    ...
    // 2.5 发布用于图优化的点云
    // 由于上一帧的点云已经被统一到了当前帧的起始时刻，匹配的是当前帧的终止位姿相对于起始位姿的转换，此处的内容是按照优化后的结果，
    // 那当前帧的点云统一到终止时刻，然后将特征点云发布出去
    void publishCloudsLast()
    {
        ...
        if (frameCount >= skipFrameNum + 1)
        {
            ...
            outlierCloudLast2.header.frame_id = "vrslidar_link"; // 原为"/camera"
            ...
            pcl::toROSMsg(*laserCloudCornerLast, laserCloudCornerLast2);
            laserCloudCornerLast2.header.stamp = cloudHeader.stamp;
            laserCloudCornerLast2.header.frame_id = "vrslidar_link"; // 原为"/camera"
            pubLaserCloudCornerLast.publish(laserCloudCornerLast2);
            ...
            pcl::toROSMsg(*laserCloudSurfLast, laserCloudSurfLast2);
            laserCloudSurfLast2.header.stamp = cloudHeader.stamp;
            laserCloudSurfLast2.header.frame_id = "vrslidar_link"; // 原为"/camera"
            pubLaserCloudSurfLast.publish(laserCloudSurfLast2);
        }
    }
    ...
    {
        // 0. 准备工作
        if (newSegmentedCloud && newSegmentedCloudInfo && newOutlierCloud && // 三个主要的回调函数是否均接收到
            std::abs(timeNewSegmentedCloudInfo - timeNewSegmentedCloud) < 0.05 &&
            std::abs(timeNewOutlierCloud - timeNewSegmentedCloud) < 0.05) // 三个回调函数的时间差不能超过0.05s，保证信息同步
        {
            // 满足正常工作的条件后清空标志位
            newSegmentedCloud = false;
            newSegmentedCloudInfo = false;
